...
if len(sys.argv) == 1:
    ...
    data_id = sys.argv[1]
# If data_id is invalid, raise an error.
assert(data_id in stuff_table.keys())
...
pkl_folder = os.path.join('/nfs/cell_a/userhome/zcc/documents/RSVP_experiment/Project_RSVP_coding_work/xdawn_MVPA_results_lr')
print(pkl_folder)
...
try:
    ...
    print('Something bad happens,\
            but it can be just File exists error which does not matter.')
...
for band in bands:
    ...
    epochs = epochs.filter(l_freq=l_freq, h_freq=h_freq, n_jobs=n_jobs, verbose=2)
    ...
    for train_index, test_index in logo.split(y_true, y_true, groups):
        ...
        # Fit and apply Xdawn denoise
        xdawn.fit(epochs[train_index])
        epochs_xdawn = xdawn.apply(epochs.copy(), event_id=['odd'])['odd']

        # Seperate train and test data
        X_train = epochs_xdawn.get_data()[train_index]
        y_train = y_true[train_index]
        X_test = epochs_xdawn.get_data()[test_index]
        ...
        for window_length, _info in time_windows.items():
            ...
            for j, w_start in enumerate(_info[0]):
                ...
                y_pred_timewindow[window_length][test_index, j] = raw_decoder.predict(_X_test)
